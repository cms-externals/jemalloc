...
TEST_BEGIN(test_thread_peak) {
	...
	assert_d_eq(err, 0, "");
	assert_u64_eq(peak, SC_SMALL_MAXCLASS, "Missed an update");
	free(ptr);
	err = mallctl("thread.peak.read", &peak, &sz, NULL, 0);
	assert_d_eq(err, 0, "");
	assert_u64_eq(peak, SC_SMALL_MAXCLASS, "Freeing changed peak");
	...
	err = mallctl("thread.peak.read", &peak, &sz, NULL, 0);
	assert_d_eq(err, 0, "");
	assert_u64_ge(peak, big_size, "Missed a peak change.");
	...
	err = mallctl("thread.peak.read", &peak, &sz, NULL, 0);
	assert_d_eq(err, 0, "");
	assert_u64_eq(0, peak, "Missed a reset.");
	...
	err = mallctl("thread.peak.read", &peak, &sz, NULL, 0);
	assert_d_eq(err, 0, "");
	assert_u64_ge(peak, nallocx(small_size, 0) * nallocs,
