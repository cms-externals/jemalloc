...
TEST_BEGIN(test_peak) {
	...
	assert_u64_eq(0, peak_max(&peak),
	    "Peak should be zero at initialization");
	peak_update(&peak, 100, 50);
	assert_u64_eq(50, peak_max(&peak),
	    "Missed update");
	peak_update(&peak, 100, 100);
	assert_u64_eq(50, peak_max(&peak), "Dallocs shouldn't change peak");
	peak_update(&peak, 100, 200);
	assert_u64_eq(50, peak_max(&peak), "Dallocs shouldn't change peak");
	peak_update(&peak, 200, 200);
	assert_u64_eq(50, peak_max(&peak), "Haven't reached peak again");
	peak_update(&peak, 300, 200);
	assert_u64_eq(100, peak_max(&peak), "Missed an update.");
	peak_set_zero(&peak, 300, 200);
	assert_u64_eq(0, peak_max(&peak), "No effect from zeroing");
	peak_update(&peak, 300, 300);
	assert_u64_eq(0, peak_max(&peak), "Dalloc shouldn't change peak");
	peak_update(&peak, 400, 300);
	assert_u64_eq(0, peak_max(&peak), "Should still be net negative");
